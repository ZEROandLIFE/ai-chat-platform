# AI Chat Platform - 开发计划

## 项目概述

本项目是一个模仿DeepSeek/豆包的AI对话平台Web应用，支持多轮对话、历史记录管理、文件上传、Markdown渲染、代码高亮等功能，使用纯Mock数据实现对话逻辑。

---

### 阶段1：项目初始化与基础配置

**目标**：完成项目初始化，配置开发环境，预留后续功能扩展空间

**技术栈**：

| 分类 | 技术 | 说明 |
|:-----|:-----|:-----|
| 前端 | Vue 3.4+ | 前端框架 |
| 前端 | TypeScript | 类型系统 |
| 前端 | Vite | 构建工具 |
| 前端 | Pinia | 状态管理 |
| 前端 | Vue Router | 路由管理 |
| 前端 | marked | Markdown解析器（预留，未安装） |
| 前端 | highlight.js | 代码高亮库（预留，未安装） |
| 前端 | DOMPurify | HTML净化库（预留，未安装） |

**前端任务**：
1. 清理现有HelloWorld组件
2. 创建TypeScript类型定义文件（预留所有功能接口）
3. 配置Pinia状态管理
4. 配置Vue Router路由
5. 创建项目目录结构
6. ~~安装Markdown渲染相关依赖（marked, highlight.js, dompurify）~~

**目录结构**：
```
frontend/src/
├── components/
│   ├── chat/           # 预留
│   ├── sidebar/        # 预留
│   ├── common/         # 预留
│   └── markdown/       # 预留
├── views/
│   └── layout/
│       └── MainLayout.vue   # 主布局组件
├── stores/
│   ├── theme.ts        # ✅ 主题状态管理
│   ├── chat.ts         # ✅ 对话状态管理
│   └── index.ts        # ✅ Store导出
├── types/
│   ├── chat.ts         # ✅ Message、Conversation、ChatState、MOCK_RESPONSES
│   ├── theme.ts        # ✅ ThemeState
│   ├── model.ts        # ✅ Model、AVAILABLE_MODELS
│   ├── common.ts       # ✅ UploadedFile、generateId、generateConversationTitle
│   └── index.ts        # ✅ 类型导出
├── styles/
│   └── style.css       # ✅ 全局样式
├── utils/              # 预留（阶段3实现）
├── router/
│   └── index.ts        # ✅ Vue Router配置
└── App.vue             # ✅ 根组件
```

**已创建文件**：

| 文件路径 | 说明 |
|:---------|:-----|
| stores/theme.ts | 主题状态管理（useThemeStore）- 包含mode、toggleTheme、setTheme |
| stores/chat.ts | 对话状态管理（useChatStore）- 包含对话CRUD、消息发送、Mock回复、流式效果、停止生成等 |
| stores/index.ts | Store统一导出 |
| types/chat.ts | Message、Conversation、ChatState、MOCK_RESPONSES（4种）、EXAMPLE_QUESTIONS |
| types/theme.ts | ThemeState |
| types/model.ts | Model、AVAILABLE_MODELS（3种模型） |
| types/common.ts | UploadedFile、generateId、generateConversationTitle |
| types/index.ts | 类型统一导出 |
| router/index.ts | Vue Router配置（根路径→MainLayout） |
| views/layout/MainLayout.vue | 主布局组件（侧边栏+聊天区域+消息展示） |
| style.css | 全局样式（日间/夜间主题CSS变量） |
| main.ts | 应用入口（Pinia+Router配置） |
| App.vue | 根组件 |

**数据模型**：

```typescript
// types/chat.ts
interface Message {
  id: string;
  content: string;
  role: 'user' | 'assistant';
  timestamp: Date;
  isStreaming?: boolean;
  isEdited?: boolean;
  quotedMessage?: Message;
  isStopped?: boolean;
}

interface Conversation {
  id: string;
  title: string;
  messages: Message[];
  createdAt: Date;
  updatedAt: Date;
  model: string;
}

interface ChatState {
  currentConversationId: string | null;
  conversations: Conversation[];
  isLoading: boolean;
  isGenerating: boolean;
  quotedMessage: Message | null;
  editingMessageId: string | null;
}

// types/theme.ts
interface ThemeState {
  mode: 'light' | 'dark';
}

// types/model.ts
interface Model {
  id: string;
  name: string;
  description: string;
}

// types/common.ts
interface UploadedFile {
  id: string;
  name: string;
  size: number;
  type: string;
  uploadTime: Date;
}
```

**功能特性**：
- Vue3 + TypeScript项目架构
- 组件化开发结构
- Pinia状态管理（useThemeStore、useChatStore）
- Vue Router路由配置
- 类型文件分类管理（chat/theme/model/common）
- 预留Markdown渲染、代码高亮、消息引用等功能的类型定义
- 基础UI布局（侧边栏+聊天区域）
- 日间/夜间主题切换
- 新建对话、历史记录切换
- 对话标题自动生成（仅默认标题时）
- Mock数据回复（4种不同回复）
- localStorage持久化（对话+主题）
- 打字机效果（前端模拟）
- 停止生成功能

**完成状态**：已完成

---

### 阶段2：布局与主题系统

**目标**：实现整体布局框架和日间/夜间主题切换

**前端任务**：

1. 创建主布局组件
2. 创建侧边栏组件（集成在MainLayout.vue中，未拆分）
3. 创建头部组件（集成在MainLayout.vue中，未拆分）
4. 实现主题切换功能
5. 创建主题Store（stores/theme.ts）

**布局结构**：
```
┌─────────────────────────────────────────┐
│  Header (☰ + 新建对话 + AI Chat)        │
├──────┬──────────────────────────────────┤
│      │                                  │
│ Side │      Chat Messages Area          │
│ bar  │      (可滚动对话消息列表)          │
│      │                                  │
│      ├──────────────────────────────────┤
│      │  Input Area (输入框 + 发送按钮)  │
└──────┴──────────────────────────────────┘
```

**侧边栏功能**：
- 新建对话按钮
- 历史对话列表
- 对话标题编辑（双击或点击编辑按钮）
- 删除对话按钮
- 模型选择下拉框（预留UI，未实现）
- 文件上传按钮（预留UI，未实现）
- 主题切换按钮

**主题系统**：
- 日间模式：白色背景 (#ffffff) + 黑色文字
- 夜间模式：深灰背景 (#1a1a1a) + 白色文字
- 侧边栏日间：浅灰背景 (#f8f9fa)
- 侧边栏夜间：深灰背景 (#1e1e1e)
- 主题切换使用Emoji（☀️/🌙）
- 本地存储主题偏好（localStorage）

**实现细节**：

```typescript
// stores/theme.ts
export const useThemeStore = defineStore('theme', () => {
  const mode = ref<'light' | 'dark'>(
    (localStorage.getItem('theme-mode') as 'light' | 'dark') || 'light'
  );

  const toggleTheme = () => {
    mode.value = mode.value === 'light' ? 'dark' : 'light';
  };

  watch(mode, (newMode) => {
    localStorage.setItem('theme-mode', newMode);
    document.documentElement.setAttribute('data-theme', newMode);
  }, { immediate: true });

  return { mode, toggleTheme, setTheme };
});
```

```css
/* style.css */
:root {
  color-scheme: light dark;
}

.main-layout {
  background: #ffffff;
  color: #000000;
}

.main-layout.dark {
  background: #1a1a1a;
  color: #ffffff;
}
```

**功能特性**：
- 左侧固定侧边栏（可收起）
- 右侧对话主区域
- 日间/夜间主题切换
- 响应式布局
- 主题状态持久化
- 对话标题可编辑（双击或编辑按钮）
- 侧边栏收起时Header显示新建对话按钮
- 收起侧边栏时显示折叠按钮
- 历史对话列表（点击切换）
- 消息气泡展示（用户右对齐/AI左对齐）
- 消息时间戳显示
- 输入框+发送按钮
- Enter发送消息

**组件结构（实际）**：

| 组件 | 文件 | 说明 |
|:-----|:-----|:-----|
| MainLayout | views/layout/MainLayout.vue | 主布局（包含所有UI） |
| Sidebar | - | 集成在MainLayout中 |
| Header | - | 集成在MainLayout中 |
| ChatMessages | - | 集成在MainLayout中 |
| ChatInput | - | 集成在MainLayout中 |

**完成状态**：已完成

---

### 阶段3：Mock API与拦截器

**目标**：建立独立的Mock数据层、API服务、请求拦截器，实现流式Mock响应

**前置条件**：需要在阶段4对话核心之前完成，因为对话功能依赖API层

**前端任务**：

1. 创建独立的Mock数据文件（utils/mockData.ts）
2. 创建Mock API服务（utils/mockApi.ts）
3. 创建请求拦截器（utils/interceptor.ts）
4. 创建API类型定义（types/api.ts）
5. 实现流式Mock响应（模拟SSE）
6. 创建API统一导出（utils/api.ts）

**目录结构**：
```
frontend/src/
├── utils/
│   ├── mockData.ts      # Mock数据（4种回复内容）
│   ├── mockApi.ts       # Mock API服务（chat/completion接口）
│   ├── interceptor.ts   # 请求/响应拦截器
│   └── api.ts           # API统一导出
├── types/
│   └── api.ts           # API相关类型定义
```

**Mock数据类型**：

```typescript
// types/api.ts
export interface ChatMessage {
  role: 'user' | 'assistant' | 'system';
  content: string;
}

export interface ChatCompletionRequest {
  messages: ChatMessage[];
  model?: string;
  stream?: boolean;
}

export interface ChatCompletionResponse {
  id: string;
  model: string;
  choices: {
    index: number;
    message: ChatMessage;
    finish_reason: string;
  }[];
  usage?: {
    prompt_tokens: number;
    completion_tokens: number;
    total_tokens: number;
  };
}

export interface StreamChunk {
  id: string;
  choices: {
    index: number;
    delta: Partial<ChatMessage>;
    finish_reason?: string;
  }[];
}
```

**Mock数据文件**：

```typescript
// utils/mockData.ts
export const MOCK_RESPONSES: string[] = [
  `你好！我是AI助手。我可以帮你解答各种问题。

## 功能特性

- **代码示例**：我可以展示代码
- **列表支持**：支持有序和无序列表
- **引用块**：可以引用内容

### 代码示例

\`\`\`javascript
function greet(name) {
  console.log(\`Hello, \${name}!\`);
}

greet('World');
\`\`\``,

  `这是一个不同的回复示例。

\`\`\`python
def fibonacci(n):
    if n <= 1:
        return n
    return fibonacci(n-1) + fibonacci(n-2)

print(fibonacci(10))
\`\`\``
];

export const EXAMPLE_QUESTIONS: string[] = [
  '如何学习编程？',
  '解释一下什么是机器学习',
  '写一个Python快速排序算法',
  '如何提高工作效率？'
];
```

**Mock API服务**：

```typescript
// utils/mockApi.ts
import type { ChatCompletionRequest, StreamChunk } from '../types/api';
import { MOCK_RESPONSES } from './mockData';

export async function chatCompletion(
  request: ChatCompletionRequest
): Promise<ChatCompletionResponse> {
  const randomResponse = MOCK_RESPONSES[Math.floor(Math.random() * MOCK_RESPONSES.length)];

  return {
    id: 'mock-' + Date.now(),
    model: request.model || 'gpt-3.5',
    choices: [{
      index: 0,
      message: {
        role: 'assistant',
        content: randomResponse
      },
      finish_reason: 'stop'
    }]
  };
}

export async function* chatCompletionStream(
  request: ChatCompletionRequest
): AsyncGenerator<StreamChunk> {
  const response = MOCK_RESPONSES[Math.floor(Math.random() * MOCK_RESPONSES.length)];
  const chunkSize = 10; // 每chunk字符数
  const delay = 50; // ms延迟

  for (let i = 0; i < response.length; i += chunkSize) {
    const chunkContent = response.slice(i, i + chunkSize);
    yield {
      id: 'mock-' + Date.now(),
      choices: [{
        index: 0,
        delta: { content: chunkContent }
      }]
    };

    await new Promise(resolve => setTimeout(resolve, delay));
  }

  yield {
    id: 'mock-' + Date.now(),
    choices: [{
      index: 0,
      delta: {},
      finish_reason: 'stop'
    }]
  };
}
```

**拦截器逻辑**：

```typescript
// utils/interceptor.ts
class InterceptorManager {
  private requestInterceptors: Function[] = [];
  private responseInterceptors: Function[] = [];

  useRequestInterceptor(fn: Function) {
    this.requestInterceptors.push(fn);
  }

  useResponseInterceptor(fn: Function) {
    this.responseInterceptors.push(fn);
  }

  async interceptRequest(config: any) {
    let result = config;
    for (const interceptor of this.requestInterceptors) {
      result = await interceptor(result);
    }
    return result;
  }

  async interceptResponse(response: any) {
    let result = response;
    for (const interceptor of this.responseInterceptors) {
      result = await interceptor(result);
    }
    return result;
  }
}

export const interceptorManager = new InterceptorManager();

// 日志拦截器示例
interceptorManager.useRequestInterceptor((config: any) => {
  console.log('[Request]', config);
  return config;
});

interceptorManager.useResponseInterceptor((response: any) => {
  console.log('[Response]', response);
  return response;
});
```

**流式Mock响应原理**：

```
chatStore.sendMessage("你好")
        ↓
mockApi.chatCompletionStream(request)
        ↓
for...of循环 + yield + setTimeout
        ↓
逐chunk返回: "你好！..." → "我是..." → "AI..." → "助手..."
        ↓
ChatStore逐chunk更新消息内容
        ↓
前端显示打字机效果
```

**功能特性**：
- 独立的Mock数据文件
- 统一的API服务层
- 请求/响应拦截器
- 支持流式和非流式两种Mock模式
- 流式Mock使用async generator实现
- 模拟真实API的响应格式
- 便于后续替换为真实API

**完成状态**：已完成

---

### 阶段4：对话核心功能

**目标**：实现对话消息展示、发送、流式Mock回复功能

**前置条件**：依赖阶段3 Mock API层

**前端任务**：

1. 创建消息气泡组件
2. 创建消息列表组件
3. 创建输入框组件
4. 创建加载动画组件
5. 更新对话Store调用Mock API
6. 实现流式消息更新
7. 实现滚动到底部功能
8. 实现停止生成功能

**数据流**：

```
用户发送消息
    ↓
ChatStore.sendMessage()
    ↓
chatCompletionStream() from Mock API
    ↓
for...of 逐chunk获取数据
    ↓
更新消息内容 (isStreaming: true)
    ↓
前端实时显示打字效果
    ↓
完成时 isStreaming: false
```

**功能特性**：
- 消息发送与展示
- 用户消息右对齐，AI消息左对齐
- 消息时间戳显示
- 流式AI回复（打字机效果）
- 自动滚动到最新消息
- 支持Enter发送
- 发送时输入框禁用
- 停止生成功能
- 右侧消息导航栏（DeepSeek风格）
  - 显示用户提问坐标点
  - 鼠标悬停显示问题预览
  - 点击跳转对应消息位置
  - 滚动时高亮当前位置

**完成状态**：已完成

---

### 阶段5：历史记录与文件功能

**目标**：实现对话历史管理、新建对话、文件上传功能

**前端任务**：

1. 创建历史对话列表组件
2. 创建新建对话按钮组件
3. 创建文件上传组件
4. 实现对话历史持久化
5. 实现新建对话功能
6. 实现切换对话功能
7. 实现删除对话功能
8. 实现文件上传UI（仅UI，无实际上传）

**功能特性**：
- 新建对话
- 历史对话列表展示
- 切换历史对话
- 删除历史对话
- 对话标题自动生成（基于首条消息，仅默认标题时）
- 文件上传按钮（仅UI展示）
- 已上传文件列表展示
- localStorage持久化存储

**完成状态**：已完成

---

### 阶段6：Markdown渲染与代码高亮

**目标**：实现AI回复的Markdown渲染和代码块语法高亮

**前端任务**：

1. 创建Markdown渲染组件
2. 创建代码块组件
3. 配置marked解析器
4. 配置highlight.js代码高亮
5. 配置DOMPurify防XSS攻击
6. 实现Markdown样式
7. 实现代码块复制功能
8. 更新Mock数据包含Markdown内容

**Markdown支持**：
- 标题（h1-h6）
- 列表（有序、无序）
- 代码块（带语法高亮）
- 行内代码
- 加粗、斜体
- 引用块
- 表格
- 链接

**代码高亮支持**：
- JavaScript/TypeScript
- Python
- Java
- C/C++
- Go
- Rust
- HTML/CSS
- SQL
- Shell
- JSON/YAML

**功能特性**：
- Markdown实时渲染
- 代码块语法高亮
- 代码块一键复制
- XSS防护
- 自定义Markdown样式
- 适配日间/夜间主题

**完成状态**：已完成

---

### 阶段7：AI生成控制

**目标**：实现AI生成时用户的暂停/继续控制能力，使用真实后端API实现流式传输

**前端任务**：
1. 实现与后端API的流式响应连接（SSE）
2. 移除模拟打字机动画，使用真实流式响应
3. 实现AI生成时的暂停控制
4. 更新ChatStore使用真实API调用

**后端任务**：
1. 实现流式响应接口（/api/conversations/:id/message/stream）
2. 使用SSE（Server-Sent Events）传输数据
3. 代理LLM API的流式响应

**功能特性**：
- AI生成时，输入框显示"停止"按钮
- AI生成完成或停止后，输入框显示"发送"按钮
- 用户可以主动停止AI生成
- 真实流式响应（非模拟打字机效果）
- SSE实时数据传输

**完成状态**：已完成

---

### 阶段8：高级交互功能

**目标**：实现重新生成、消息引用、复制、删除等高级交互功能

**前置条件**：依赖阶段4对话核心和阶段6 Markdown渲染

**前端任务**：

1. 创建消息操作组件（重新生成、引用、复制、删除）
2. 更新消息气泡组件（添加操作按钮）
3. 更新ChatStore（添加相关状态和方法）
4. 实现重新生成逻辑
5. 实现消息引用逻辑
6. 实现消息复制功能
7. 实现消息删除功能

**功能详解**：

#### 重新生成
- AI消息下方显示"重新生成"按钮
- 点击后删除当前AI回复
- 重新发送最后一条用户消息
- 生成新的AI回复（使用流式）

#### 消息引用
- 消息下方显示"引用"按钮
- 点击后在输入框上方显示引用内容
- 发送消息时附带引用信息
- 引用的消息在对话中高亮显示

#### 消息复制
- 消息下方显示"复制"按钮
- 点击后复制消息内容到剪贴板
- 显示复制成功提示

#### 消息删除
- 消息下方显示"删除"按钮
- 点击后确认删除消息
- 删除后更新对话历史

**功能特性**：
- 重新生成回复（流式）
- 引用历史消息
- 复制消息内容
- 删除消息
- 消息操作按钮（点击展开菜单）
- 操作确认提示

**完成状态**：已完成

---

### 阶段9：文件预览面板

**目标**：实现文件预览功能，点击上传的文件在右侧面板中进行预览

**前置条件**：依赖阶段5文件上传功能

**前端任务**：

1. 创建文件预览面板组件（FilePreviewPanel）
2. 在主布局中添加文件预览区域
3. 实现文件点击预览功能
4. 实现预览面板展开/收起动画
5. 更新上传文件列表，添加点击事件
6. 实现预览面板头部（文件名、关闭按钮）
7. 实现文件类型图标显示
8. 安装依赖库（pdfjs-dist、mammoth）
9. 实现文件内容读取服务（FileReaderService）

**功能详解**：

#### 文件预览面板
- 右侧可收起的预览面板
- 点击上传文件列表中的文件，在预览面板中显示内容
- 支持PDF、Word、TXT等文件类型预览
- 预览面板可收起/展开
- 显示文件名和文件大小

#### 文件内容显示
- 使用前端技术直接读取并预览上传的文件
- PDF文件：使用pdf.js渲染PDF页面
- Word文件：使用mammoth.js提取文本内容
- TXT文件：使用FileReader直接读取
- 图片文件：创建Blob URL显示

**技术方案**：
- PDF预览：`pdfjs-dist` 库
- Word解析：`mammoth` 库
- 文件读取：原生 `FileReader` API
- 图片预览：原生 `URL.createObjectURL()`

**功能特性**：
- 右侧文件预览面板
- 文件点击预览（真实文件内容）
- 预览面板展开/收起
- 文件类型识别
- 支持PDF、Word、TXT、图片预览

**完成状态**：未完成

---

### 阶段10：文档渲染引擎

**目标**：实现文档内容的渲染和分页展示

**前置条件**：依赖阶段9文件预览面板

**前端任务**：

1. 创建文档渲染组件（DocumentRenderer）
2. 实现分页展示功能（PDF专用）
3. 添加页码导航
4. 实现缩放功能（适合/缩小/放大）
5. 添加文档工具栏（页码、缩放控制）
6. 实现不同文件类型的渲染样式
7. 实现PDF页面渲染
8. 实现Word/TXT文本渲染

**功能详解**：

#### 分页展示
- 文档内容分页显示
- 底部页码导航
- 上一页/下一页按钮
- 当前页码显示

#### 缩放控制
- 适合宽度按钮
- 缩小按钮（-）
- 放大按钮（+）
- 缩放比例显示

#### 文档工具栏
- 文件名显示
- 页码导航
- 缩放控制
- 关闭预览按钮

**功能特性**：
- 文档分页展示
- 页码导航
- 缩放控制
- 文档工具栏
- 适配日间/夜间主题

**完成状态**：未完成

---

### 阶段11：划词提问功能

**目标**：实现文档预览中选中文本，自动带入对话输入框进行提问

**前置条件**：依赖阶段9文件预览面板和阶段10文档渲染引擎

**前端任务**：

1. 实现文本选中监听
2. 创建选中文字浮动菜单（悬浮工具栏）
3. 实现"提问"按钮功能
4. 将选中文字带入对话输入框
5. 添加提问模板（如"解释一下这段内容"）
6. 实现选中文字高亮显示
7. 更新ChatStore添加选中文字状态

**功能详解**：

#### 文本选中
- 监听文档区域的鼠标选中事件
- 选中文本后显示浮动工具栏
- 支持点击其他区域取消选中

#### 浮动工具栏
- 选中文本上方显示悬浮菜单
- 包含"提问"按钮
- 支持自定义提问模板
- 点击提问后自动填充到输入框

#### 提问功能
- 选中文字自动带入输入框
- 支持添加提问模板前缀
- 自动聚焦到输入框
- 可选发送或手动编辑后发送

**功能特性**：
- 文档文本选中监听
- 浮动工具栏显示
- 选中文字带入对话
- 提问模板支持
- 选中高亮显示

**完成状态**：未完成

---

## 注意事项

1. **纯Mock数据**：所有对话回复均为固定内容，不连接真实API
2. **主题系统**：只使用白色和黑色，实现日间/夜间模式
3. **文件上传**：仅实现UI展示，不实际上传文件
4. **代码规范**：使用TypeScript，组件采用Composition API
5. **状态持久化**：使用localStorage存储对话历史和主题偏好
6. **Markdown安全**：使用DOMPurify防止XSS攻击
7. **扩展性**：所有数据模型和组件设计预留扩展空间
8. **流式响应**：使用async generator实现模拟流式，无需后端支持
